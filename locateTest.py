...
#########  Function to draw line from float numbers  #########
def myLine(myim, val1, val2, val3, val4):
    im = cv2.line(myim, (int(val1), int(val2)), (int(val3), int(val4)), (255, 0, 255))
...
for count in range(len(contours)):
    ...
    if(area >33000 and area < 36000):
        ...
        drawID = 2
    if(drawID <> 0):
######### approximate contour as polygon with 2.5% deviation  #########

        epsilon = 0.0025*cv2.arcLength(cnt, True)            #([contour], [closed contour(T/F)])
...
######### approximate contour as bounding rectangle  #########
        ...
        maxVal = max(wVal, hVal)
        minVal = min(wVal, hVal)
        
        sinAngleVal = math.sin(rect[2]*3.14159/180)
        cosAngleVal = math.cos(rect[2]*3.14159/180)
        print "x: " + str(xVal)
        print "y: " + str(yVal)
        print "w: " + str(wVal)
        print "h: " + str(hVal)
        print "angle: " + str(angleVal)
        print "sinAngle: " + str(sinAngleVal)
        print "cosAngle: " + str(cosAngleVal)
        print "\n"
        ...
        ######### Draw a longer line across width #########

        if(wVal == minVal):     #width is smaller than height
            myLine(im, xVal+40*sinAngleVal, yVal-40*cosAngleVal, xVal-40*sinAngleVal, yVal+40*cosAngleVal)  #line through height
            myLine(im, xVal-20*cosAngleVal, yVal-20*sinAngleVal, xVal+20*cosAngleVal, yVal+20*sinAngleVal)  #line through width

        if(hVal == minVal):     #width is larger than height
            myLine(im, xVal+20*sinAngleVal, yVal-20*cosAngleVal, xVal-20*sinAngleVal, yVal+20*cosAngleVal)  #line through height
            myLine(im, xVal-40*cosAngleVal, yVal-40*sinAngleVal, xVal+40*cosAngleVal, yVal+40*sinAngleVal)  #line through width
            #add something here to convert to width and height (add 90 to angle?)
